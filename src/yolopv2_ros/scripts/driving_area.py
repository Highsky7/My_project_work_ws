...
ROS 노드:
 ...
 2) 주행 가능 영역(Driving Area) 세그멘테이션도 함께 표시
 3) 이진화 마스크 + Thinning (얇은 선) => 차선
 4) (A) 실제 카메라 컬러 영상 + (주행 영역 + 차선) 오버레이
 5) (B) BEV 변환
 6) 결과를 ROS 토픽(컬러) 퍼블리시
 7) 실시간 창 및 저장된 영상에서도 컬러 BEV 형태로 얇은 차선 + 주행 영역 확인
"""
...
def detect_and_publish(opt, pub_img):
    """
    Lane/DrivingArea 세그멘테이션 -> Thinning(차선) + Overlay + BEV -> 퍼블리시 + 저장
    """
    ...
    for path_item, img, im0s, vid_cap in dataset:
        ...
        # 모델 추론
        # YOLOPv2는 보통 [det_out, seg_out], ll_out = model(img_t) 형태
        # 여기서는 [_, _], seg, ll 라고 받는다고 가정
        [_, _], seg, ll = model(img_t)
        t2 = time_synchronized()

        # Lane Line 세그멘테이션 마스크
        ll_seg_mask = lane_line_mask(ll)  # (H, W) 이진 마스크
        ll_seg_mask = (ll_seg_mask > 0).astype(np.uint8) * 255

        # Thinning(차선만 얇게)
        thin_mask = ximgproc.thinning(ll_seg_mask, thinningType=ximgproc.THINNING_ZHANGSUEN)

        # Driving Area(주행 가능 영역) 세그멘테이션 마스크
        # seg가 주행 영역이라 가정 => threshold 적용
        # (seg가 1채널이고 0 or 1로 나올 수도, 0~255이거나, float형식일 수도 있음)
        da_seg_mask = driving_area_mask(seg)

        inf_time.update(t2 - t1, img_t.size(0))

        # (A) 실제 컬러 영상 오버레이
        overlay_img = im0s.copy()  # BGR 원본 카메라 영상

        # 1) 주행 영역(녹색)
        overlay_img[da_seg_mask == 255] = (0, 255, 0)
        ...
        # 실시간 화면 표시
        cv2.imshow('YOLOPv2 Lane+DrivingArea BEV', bev_img)
            ...
            if dataset.mode == 'image':
                ...
                if nonlocal_vid_path != save_path:
                    ...
                    if vid_cap:
                        ...
                        wv = int(vid_cap.get(cv2.CAP_PROP_FRAME_WIDTH)) or bev_img.shape[1]
                        hv = int(vid_cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or bev_img.shape[0]
                    else:
                        fps = 30
                        wv, hv = bev_img.shape[1], bev_img.shape[0]

                    rospy.loginfo(f"[INFO] 비디오 저장 시작: {nonlocal_vid_path} "
                                  f"(FPS={fps}, size=({wv},{hv}))")
