...
# 메인 처리 함수
def detect_and_publish(opt, pub_mask, pub_steering, pub_lane_status, pub_lane_path):
    ...
    def process_frame(im0s):
        ...
        if lane_detected:
            ...
            lane_path_msg = Path()
            lane_path_msg.header.frame_id = "velodyne"
            lane_path_msg.header.stamp = rospy.Time.now()
            for pt in shifted_poly_points:
                x_v, y_v = image_to_vehicle(pt)
                ps = PoseStamped()
                ps.header = lane_path_msg.header
                ps.pose.position.x = x_v  # Forward (m)
                ps.pose.position.y = y_v  # Left (m)
                ps.pose.position.z = 0.0
                ps.pose.orientation.w = 1.0  # No orientation info
                lane_path_msg.poses.append(ps)
            pub_lane_path.publish(lane_path_msg)
            rospy.loginfo("[INFO] Published lane_path with %d points", len(lane_path_msg.poses))
...
def ros_main():
    ...
    pub_lane_path = rospy.Publisher('lane_path', Path, queue_size=1)  # New publisher
    detect_and_publish(opt, pub_mask, pub_steering, pub_lane_status, pub_lane_path)
    rospy.spin()
